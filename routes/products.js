...
// api/products/subcategories/all
router.get('/subcategories/all', async (req, res) => {
  // Returns all subcategories inside database
  const results = await Product.find(
    {
      endDateAsNum: { $gte: new Date().getTime() },
    },
  ...
  // removes id's from the value
  results.forEach((res) => {
    justCategories.push(res.subCategory);
  });

  // turns a matrix array to a single array
  const merged = [].concat.apply([], justCategories);
  ...
  res.send(merged.filter(onlyUnique));
});
        ...
        for (let i = 0; i < allResults.length; i++) {
          if (finalResults.length == 0) {
            ...
            for (let j = 0; j < finalResults.length; j++) {
              if (
                JSON.stringify(allResults[i]) ===
                JSON.stringify(finalResults[j])
              ) {
                found = true;
              }
            }
